...
{
	const auto existing (std::find_if (epochs_m.begin (), epochs_m.end (), [&link_a] (auto const & item_a) { return item_a.second.link == link_a; }));
...
{
	const auto head_epoch = std::underlying_type_t<nano::epoch> (epoch_a);
	const bool is_valid_epoch (head_epoch >= std::underlying_type_t<nano::epoch> (nano::epoch::epoch_0));
...
std::underlying_type_t<nano::epoch> nano::normalized_epoch (nano::epoch epoch_a)
{
	...
	const auto start = std::underlying_type_t<nano::epoch> (nano::epoch::epoch_0);
	const auto end = std::underlying_type_t<nano::epoch> (epoch_a);
