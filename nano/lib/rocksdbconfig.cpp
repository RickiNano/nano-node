...
nano::error nano::rocksdb_config::serialize_toml (nano::tomlconfig & toml) const
{
	...
	toml.put ("read_cache", read_cache, "Amount of megabytes per table allocated to read cache. Valid range is 1 - 1024. Default is 64.\nCarefully monitor memory usage if non-default values are used\ntype:long");
	toml.put ("write_cache", write_cache, "Amount of megabytes per table allocated to write cache. Valid range is 1 - 1024. Default is 64.\nCarefully monitor memory usage if non-default values are used\ntype:long");
...
nano::error nano::rocksdb_config::deserialize_toml (nano::tomlconfig & toml)
{
	...
	if (write_cache < 1 || write_cache > 1024)
	{
		toml.get_error ().set ("write_cache must be between 1 and 1024 MB");
	}
