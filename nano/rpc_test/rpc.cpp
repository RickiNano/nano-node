...
TEST (rpc, unchecked)
{
	...
	ASSERT_TIMELY_EQ (10s, node->unchecked.count (), 2);
	boost::property_tree::ptree request;
...
TEST (rpc, unchecked_get)
{
	...
	ASSERT_TIMELY_EQ (10s, node->unchecked.count (), 1);
	boost::property_tree::ptree request{};
...
TEST (rpc, unchecked_clear)
{
	...
	// Waits for the open block to get saved in the database
	ASSERT_TIMELY_EQ (10s, node->unchecked.count (), 0);
}
...
TEST (rpc, epoch_upgrade)
{
	...
	auto response (wait_response (system, rpc_ctx, request));
	ASSERT_EQ ("1", response.get<std::string> ("started"));
	ASSERT_TIMELY_EQ (10s, node->store.account.count (node->store.tx_begin_read ()), 4);
	// Check upgrade
	{
		auto transaction (node->store.tx_begin_read ());
	...
	ASSERT_TIMELY_EQ (10s, node->store.account.count (node->store.tx_begin_read ()), 5);
	// Check upgrade
	{
		auto transaction (node->store.tx_begin_read ());
...
TEST (rpc, epoch_upgrade_multithreaded)
{
	...
	auto response (wait_response (system, rpc_ctx, request));
	ASSERT_EQ ("1", response.get<std::string> ("started"));
	ASSERT_TIMELY_EQ (5s, node->store.account.count (node->store.tx_begin_read ()), 4);
	// Check upgrade
	{
		auto transaction (node->store.tx_begin_read ());
		ASSERT_EQ (4, node->store.account.count (transaction));
	...
	auto response2 (wait_response (system, rpc_ctx, request));
	ASSERT_EQ ("1", response2.get<std::string> ("started"));
	ASSERT_TIMELY_EQ (5s, node->store.account.count (node->store.tx_begin_read ()), 5);
	// Check upgrade
	{
		auto transaction (node->store.tx_begin_read ());
		ASSERT_EQ (5, node->store.account.count (transaction));
