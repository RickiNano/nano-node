...
TEST (rpc, decimal_nano_to_raw_leading_dot)
{
	nano::test::system system;
	auto node1 = add_ipc_enabled_node (system);
	auto const rpc_ctx = add_rpc (system, node1);
...
TEST (rpc, decimal_nano_to_raw_one_raw)
{
	nano::test::system system;
	auto node1 = add_ipc_enabled_node (system);
	auto const rpc_ctx = add_rpc (system, node1);
...
TEST (rpc, decimal_nano_to_raw_precision_overflow)
{
	nano::test::system system;
	auto node1 = add_ipc_enabled_node (system);
	auto const rpc_ctx = add_rpc (system, node1);
	boost::property_tree::ptree request1;
	request1.put ("action", "decimal_nano_to_raw");
	request1.put ("amount", "0.0000000000000000000000000000001");
	auto response (wait_response (system, rpc_ctx, request1));
	ASSERT_EQ (std::error_code (nano::error_common::invalid_amount).message (), response.get<std::string> ("error"));
}

TEST (rpc, decimal_nano_to_raw_negative_value)
{
	nano::test::system system;
	auto node1 = add_ipc_enabled_node (system);
	auto const rpc_ctx = add_rpc (system, node1);
	boost::property_tree::ptree request1;
	request1.put ("action", "decimal_nano_to_raw");
	request1.put ("amount", "-5");
	auto response (wait_response (system, rpc_ctx, request1));
	ASSERT_EQ (std::error_code (nano::error_common::invalid_amount).message (), response.get<std::string> ("error"));
}

TEST (rpc, decimal_nano_to_raw_invalid_input)
{
	nano::test::system system;
	auto node1 = add_ipc_enabled_node (system);
	auto const rpc_ctx = add_rpc (system, node1);
	boost::property_tree::ptree request1;
	request1.put ("action", "decimal_nano_to_raw");
	request1.put ("amount", "7.234p54");
	auto response (wait_response (system, rpc_ctx, request1));
	ASSERT_EQ (std::error_code (nano::error_common::invalid_amount).message (), response.get<std::string> ("error"));
}

TEST (rpc, decimal_nano_to_raw_zero_as_decimal)
{
	nano::test::system system;
	auto node1 = add_ipc_enabled_node (system);
	auto const rpc_ctx = add_rpc (system, node1);
	boost::property_tree::ptree request1;
	request1.put ("action", "decimal_nano_to_raw");
	request1.put ("amount", "0.0");
	auto response (wait_response (system, rpc_ctx, request1));
	ASSERT_EQ ("0", response.get<std::string> ("amount"));
}

TEST (rpc, decimal_nano_to_raw_integer_only)
{
	nano::test::system system;
	auto node1 = add_ipc_enabled_node (system);
	auto const rpc_ctx = add_rpc (system, node1);
...
TEST (rpc, decimal_nano_to_raw_decimals_zero)
{
	nano::test::system system;
	auto node1 = add_ipc_enabled_node (system);
	auto const rpc_ctx = add_rpc (system, node1);
...
TEST (rpc, decimal_nano_to_raw_leading_plus)
{
	nano::test::system system;
	auto node1 = add_ipc_enabled_node (system);
	auto const rpc_ctx = add_rpc (system, node1);
...
TEST (rpc, decimal_nano_to_raw_comma_seperator)
{
	nano::test::system system;
	auto node1 = add_ipc_enabled_node (system);
	auto const rpc_ctx = add_rpc (system, node1);
...
TEST (rpc, decimal_nano_to_raw_empty_string)
{
	nano::test::system system;
	auto node1 = add_ipc_enabled_node (system);
	auto const rpc_ctx = add_rpc (system, node1);
...
TEST (rpc, decimal_nano_to_raw_hex_notation)
{
	nano::test::system system;
	auto node1 = add_ipc_enabled_node (system);
	auto const rpc_ctx = add_rpc (system, node1);
...
TEST (rpc, decimal_nano_to_raw_scientific_notation)
{
	nano::test::system system;
	auto node1 = add_ipc_enabled_node (system);
	auto const rpc_ctx = add_rpc (system, node1);
