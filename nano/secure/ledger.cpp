...
nano::block_status nano::ledger::process (secure::write_transaction const & transaction, std::shared_ptr<nano::block> block, nano::verified_signatures const & verified)
{
	debug_assert (!work.validate_entry (*block) || constants.genesis == nano::dev::genesis);

	ledger_processor processor (transaction, *this, verified);
