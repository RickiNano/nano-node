...
TEST (migrations, lmdb_to_rocksdb)
{
	...
	auto data_path = node.application_path;

	// Populate ledger and get expected data
	auto expected = populate_ledger_for_migration (store);

	// Record source counts using count_exact
	...
	// Run migration
	node.migrate_lmdb_to_rocksdb (data_path);
	...
	nano::store::ledger_store rocksdb_store{ std::move (rocksdb_backend), nano::store::open_mode::read_only, system.stats, system.logger };
