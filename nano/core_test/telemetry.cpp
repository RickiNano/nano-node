...
TEST (telemetry, dos_tcp)
{
	...
	ASSERT_TIMELY_EQ (5s, node_server->stats.count (nano::stat::type::message, nano::stat::detail::telemetry_req, nano::stat::dir::in), 1);
...
TEST (telemetry, max_possible_size)
{
	...
	ASSERT_TIMELY_EQ (5s, node_server->stats.count (nano::stat::type::message, nano::stat::detail::telemetry_ack, nano::stat::dir::in), 1);
}
