...
TEST (active_transactions, inactive_votes_cache_election_start)
{
	...
	ASSERT_TIMELY_EQ (5s, node.ledger.cache.block_count, 5);
	...
	ASSERT_TIMELY_EQ (5s, node.ledger.cache.cemented_count, 5);
	...
	ASSERT_TIMELY_EQ (5s, node.ledger.cache.cemented_count, 7);
}
