...
TEST (confirmation_height, conflict_rollback_cemented)
{
	...
	auto test_mode = [] (nano::confirmation_height_mode mode_a) {
		...
		ASSERT_TIMELY_EQ (5s, (count = node1->stats.count (nano::stat::type::ledger, nano::stat::detail::rollback_failed)), 1);
...
TEST (confirmation_height, election_winner_details_clearing)
{
	auto test_mode = [] (nano::confirmation_height_mode mode_a) {
		...
		ASSERT_TIMELY_EQ (5s, node->stats.count (nano::stat::type::confirmation_observer, nano::stat::detail::inactive_conf_height, nano::stat::dir::out), 1);
