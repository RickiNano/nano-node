...
public:
	...
	void flood_block_many (std::deque<std::shared_ptr<nano::block>>, const std::function<void ()> & = nullptr, unsigned = broadcast_interval_ms);
	...
	void broadcast_confirm_req_batched_many (std::unordered_map<std::shared_ptr<nano::transport::channel>, std::deque<std::pair<nano::block_hash, nano::root>>>,
	                                         const std::function<void ()> & = nullptr, unsigned = broadcast_interval_ms, bool = false);
	void broadcast_confirm_req_many (std::deque<std::pair<std::shared_ptr<nano::block>, std::shared_ptr<std::vector<std::shared_ptr<nano::transport::channel>>>>>,
	                                 const std::function<void ()> & = nullptr, unsigned = broadcast_interval_ms);
