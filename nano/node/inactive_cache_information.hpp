...
public:
	...
	inactive_cache_information (std::chrono::steady_clock::time_point arrival, const nano::block_hash & hash, nano::account initial_rep_a, uint64_t initial_timestamp_a, const nano::inactive_cache_status & status) :
		arrival (arrival),
		hash (hash),
		status (status)
	{
