...
bool nano::block_arrival::add (nano::block_hash const & hash_a)
{
	...
	const auto inserted (arrival.get<tag_sequence> ().emplace_back (nano::block_arrival_info{ now, hash_a }));
	const auto result (!inserted.second);
	return result;
}
...
std::unique_ptr<nano::container_info_component> nano::collect_container_info (block_arrival & block_arrival, std::string const & name)
{
	...
	const auto sizeof_element = sizeof (decltype (block_arrival.arrival)::value_type);
