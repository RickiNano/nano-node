...
bool nano::bootstrap_attempt_lazy::lazy_start (nano::hash_or_account const & hash_or_account_a)
{
	...
	const std::size_t max_keys (node->flags.disable_legacy_bootstrap ? 4 * 1024 : 1024);
...
uint32_t nano::bootstrap_attempt_lazy::lazy_batch_size ()
{
	...
	{
		const auto lazy_blocks_ratio (static_cast<double> (total_blocks / lazy_blocks_count));
		if (lazy_blocks_ratio > nano::bootstrap_limits::lazy_batch_pull_count_resize_ratio)
		{
			...
			const double lazy_blocks_factor (std::pow (lazy_blocks_ratio / nano::bootstrap_limits::lazy_batch_pull_count_resize_ratio, 3.0));
			// Decreasing total block count weight as less important (sqrt)
			const double total_blocks_factor (std::sqrt (total_blocks / nano::bootstrap_limits::lazy_batch_pull_count_resize_blocks_limit));
			const uint32_t batch_count_min (node->network_params.bootstrap.lazy_max_pull_blocks / static_cast<uint32_t> (lazy_blocks_factor * total_blocks_factor));
...
void nano::bootstrap_attempt_lazy::lazy_pull_flush (nano::unique_lock<nano::mutex> & lock_a)
{
	...
	if (pulling < max_pulls)
	{
		...
		const nano::pull_info::count_t batch_count (lazy_batch_size ());
...
void nano::bootstrap_attempt_lazy::lazy_block_state (std::shared_ptr<nano::block> const & block_a, unsigned retry_limit)
{
	...
	const std::shared_ptr<nano::state_block> block_l (std::static_pointer_cast<nano::state_block> (block_a));
	if (block_l != nullptr)
	{
		const auto transaction (node->store.tx_begin_read ());
		const nano::uint128_t balance (block_l->hashables.balance.number ());
		...
		if (!link.is_zero () && !node->ledger.is_epoch_link (link) && !lazy_blocks_processed (link.as_block_hash ()) && !node->ledger.block_or_pruned_exists (transaction, link.as_block_hash ()))
		{
			...
			else if (node->ledger.block_or_pruned_exists (transaction, previous))
			{
				...
				const auto previous_balance (node->ledger.balance_safe (transaction, previous, error_or_pruned));
				if (!error_or_pruned)
				{
			...
			{
				const auto previous_balance (lazy_balances.find (previous));
...
void nano::bootstrap_attempt_lazy::lazy_block_state_backlog_check (std::shared_ptr<nano::block> const & block_a, nano::block_hash const & hash_a)
{
	...
	const auto find_state (lazy_state_backlog.find (hash_a));
	if (find_state != lazy_state_backlog.end ())
	{
		const auto next_block (find_state->second);
...
{
	debug_assert (!mutex.try_lock ());
	const auto inserted (lazy_blocks.insert (std::hash<::nano::block_hash> () (hash_a)));
	if (inserted.second)
	{
		++lazy_blocks_count;
...
{
	debug_assert (!mutex.try_lock ());
	const auto erased (lazy_blocks.erase (std::hash<::nano::block_hash> () (hash_a)));
	if (erased)
	{
		--lazy_blocks_count;
...
void nano::bootstrap_attempt_wallet::request_pending (nano::unique_lock<nano::mutex> & lock_a)
{
	...
	if (connection_l && !stopped)
	{
		...
		node->background ([connection_l, this_l, account] () {
			const auto client (std::make_shared<nano::bulk_pull_account_client> (connection_l, this_l, account));
			client->request ();
		});
	}
}
...
bool nano::bootstrap_attempt_wallet::wallet_finished ()
{
	...
	const auto more_accounts (!wallet_accounts.empty ());
...
void nano::bootstrap_attempt_wallet::run ()
{
	...
	const auto max_time (std::chrono::minutes (10));
	nano::unique_lock<nano::mutex> lock{ mutex };
