...
nano::node::node (std::shared_ptr<boost::asio::io_context> io_ctx_a, std::filesystem::path const & application_path_a, nano::node_config const & config_a, nano::work_pool & work_a, nano::node_flags flags_a, unsigned seq) :
	...
	store_impl (nano::make_store (logger, application_path_a, network_params.ledger, flags.read_only, true, config_a, config_a.block_processor_batch_max_time, config_a.backup_before_upgrade, flags.force_use_write_queue)),
	store (*store_impl),
