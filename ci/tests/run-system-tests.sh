...
# Otherwise, the parent directory of the working directory is assumed
NANO_REPO_DIR=${1:-../}
...
for script in "${NANO_SYSTEST_DIR}"/*.sh; do
    ...
    timeout $TEST_TIMEOUT bash "${script}" > "${name}.log" 2>&1 || status=$?
    cat "${name}.log"
    ...
    if [ "${status:-0}" -eq 0 ]; then
        echo "Passed: $name"
    elif [ "${status:-0}" -eq 124 ]; then
        echo "::error::Systest timed out: $name"
        overall_status=1
    else
        echo "::error::Systest failed: $name (${status:-Unknown error})"
        overall_status=1
    fi
done
